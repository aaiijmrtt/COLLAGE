...
def image(sources, destination, step, output):
	cv2.imwrite('%s.jpg' %output, util.collage(sources, destination, step))

def video(sources, destination, step, output):
	...
	while True:
		...
		for i in xrange(len(sources)):
			...
			cv2.imwrite('%s-%05d.jpg' %(output, count), util.collage(_source, _destination, step))
			if count % 10 == 0:
...
def arguments(string):
	try:
		opts, args = getopt.getopt(string, 'hvs:t:o:', ['help', 'video', 'sources=', 'times=', 'output='])
		if len(args) != 1: raise Exception()
		sources, destination, step, isvideo, output = [args[0]], args[0], 50, False, 'output'
		for opt, arg in opts:
		...
		sys.exit()
	return sources, destination, step, isvideo, output

if __name__ == '__main__':
	sources, destination, step, isvideo, output = arguments(sys.argv[1: ])
	if not isvideo:
		sources = [cv2.imread(source) for source in sources]
		destination = cv2.imread(destination)
		image(sources, destination, step, output)
	else:
		sources = [cv2.VideoCapture(source) for source in sources]
		destination = cv2.VideoCapture(destination)
		video(sources, destination, step, output)
